...
3. If you use Supabase for persistence, apply the database schema in `supabase/schema.sql` (for example in the Supabase SQL Editor).  
   ...
   If you previously applied the one-row-per-day unique index from that migration, run `supabase/migrate-multi-log-per-day.sql` once to allow **multiple check-ins per calendar day** and to create the `suggestion_events` table for insight follow-through logging.  
